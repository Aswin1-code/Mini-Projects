...
// ---------------- CSV Storage ----------------
String csvData =
"swing_id,timestamp,ax,ay,az,gx,gy,gz,peak_acc,peak_gyro,duration,spi\n";
...
async function updateData() {
  ...
  document.getElementById("time").innerText =
    "Timestamp : " + data.time;
  document.getElementById("spi").innerText =
...
peak_gyro = 0;
  ...
  if (swing) {
    ...
    if (angularVelocity > peak_gyro)
    peak_gyro = angularVelocity;

    if (totalAcc > peak_acc)
    peak_acc = totalAcc;
...
if ((totalAcc < END_THRESHOLD && angularVelocity < 60) && duration > 0.25) {
      ...
      float SPI = (0.7 * peak_gyro) + (0.3 * peak_acc);
      live_spi = SPI;
      live_duration = duration;

      // ---------- Serial Output ----------
      Serial.print(now); Serial.print(",");
      Serial.print(swing_id); Serial.print("->");
      ...
      Serial.print(peak_acc, 2); Serial.print(",");
      Serial.print(peak_gyro, 2); Serial.print(",");
