...
unsigned long sessionElapsedMs = 0;
...
const int MAX_RECORDS = 500;  //  ---------------- need to chg
...
void setup()
{
    ...
    Wire.begin(SDA_PIN, SCL_PIN);   //sda-26, scl-25
...
void readSensor()
{
    ...
    instantSpeed = angularVelocity * 0.03; // It is a gyro-derived project score.
}
...
)
{
    ...
    if (!sessionStarted)
    {
        sessionStartMs = completedTime;
        sessionStarted = true;
    }

    sessionElapsedMs =
        completedTime - sessionStartMs;
    ...
    if (recordCount < MAX_RECORDS)
    {
        ...
        records[recordCount].speed =
            speed;

        records[recordCount].impact =
            impact;
...
<div class="row">
    ...
    <span id="timestamp" class="value">0 ms</span>
</div>
...
<div class="row">
    ...
    <span id="impact" class="value">0.00 m/s²</span>
</div>

<div class="row">
    <span class="label">Duration</span>
    <span id="duration" class="value">0.00 s</span>
</div>

<div class="row">
    <span class="label">Swing Count</span>
...
// Periodically check for a newly completed swing.
// Only completed-swing values are displayed.
setInterval(
    updateData,
    200
);
...
void handleData()
{
    ...
    // Latest completed swing timestamp
    json += "\"time\":";
    json += String(sessionElapsedMs);


    // Latest completed swing speed
    json += ",\"speed\":";
    json += String(
        peakSpeed,
        2
    ...
    // Latest completed swing duration
    json += ",\"duration\":";
    json += String(
        lastSwingDuration / 1000.0,
        2
    );


    // Total completed swings
    json += ",\"count\":";
